...
if (!empty($search)) {
    $stmt = $conn->prepare("SELECT * FROM khachhang WHERE ho_ten LIKE ? OR so_dien_thoai LIKE ?");
    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-2 sidebar">
                ...
                <a href="khachhang.php" class="bg-primary"><i class="fas fa-users me-2"></i> Khách hàng</a>
            ...
            <div class="col-md-10 content">
                <h3 class="text-center fw-bold mb-4" style="font-size: 2rem;">DANH SÁCH KHÁCH HÀNG</h3>
                ...
                <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-4">
                    <form class="input-group shadow-sm rounded-pill overflow-hidden" method="GET" style="max-width: 420px;">
                        ...
                        <input type="text" name="search" class="form-control border-0" placeholder="Tìm theo tên hoặc sdt khách hàng..." value="<?= htmlspecialchars($search) ?>" aria-label="Tìm sản phẩm" aria-describedby="search-icon">
                    </form>

                    <a href="add_khachhang.php" class="btn btn-primary px-4 py-2 rounded-pill shadow-sm">
                    ...
                    <table class="table table-striped table-hover align-middle bg-white rounded shadow-sm overflow-hidden">
                        ...
                        <tbody>
                            <?php foreach ($khachhangs as $kh): ?>
                                <tr>
                                    <td><?= htmlspecialchars($kh['id_khachhang']) ?></td>
                                    <td><?= htmlspecialchars($kh['ho_ten']) ?></td>
                                    <td><?= htmlspecialchars($kh['email']) ?></td>
                                    <td><?= htmlspecialchars($kh['so_dien_thoai']) ?></td>
                                    <td>
                                        <a href="edit_khachhang.php?id=<?= urlencode($kh['id_khachhang']) ?>" class="btn btn-sm btn-outline-primary me-1">Sửa</a>
                                        <a href="delete_khachhang.php?id=<?= urlencode($kh['id_khachhang']) ?>" class="btn btn-sm btn-outline-danger" onclick="return confirm('Bạn có chắc muốn xóa khách hàng này không?')">Xóa</a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
