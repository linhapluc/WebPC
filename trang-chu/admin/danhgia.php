...
if (!empty($search)) {
    $stmt = $conn->prepare("SELECT dg.*, sp.ten_sp, kh.ho_ten FROM danhgia dg
    ...
    $stmt = $conn->query("SELECT dg.*, sp.ten_sp, kh.ho_ten FROM danhgia dg
                          JOIN sanpham sp ON dg.id_sanpham = sp.id_sanpham
                          JOIN khachhang kh ON dg.id_khachhang = kh.id_khachhang
                          ORDER BY dg.ngay_danhgia DESC");
}
    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-2 sidebar">
                ...
                <a href="danhgia.php" class="bg-primary"><i class="fas fa-star me-2"></i> Đánh giá</a>
            ...
            <div class="col-md-10 content">
                <h3 class="text-center fw-bold mb-4" style="font-size: 2rem;">DANH SÁCH ĐÁNH GIÁ</h3>
                ...
                <form class="input-group shadow-sm rounded-pill overflow-hidden" method="GET" style="max-width: 420px;">
                    ...
                    <input type="text" name="search" class="form-control border-0" placeholder="Tìm theo tên sản phẩm hoặc tên khách hàng..." value="<?= htmlspecialchars($search) ?>" aria-label="Tìm sản phẩm" aria-describedby="search-icon">
                </form>
            </div>
                    ...
                    <table class="table table-striped table-hover align-middle bg-white rounded shadow-sm overflow-hidden">
                        ...
                        <tbody>
                            <?php foreach ($reviews as $r): ?>
                                <tr>
                                    <td><?= htmlspecialchars($r['id_danhgia']) ?></td>
                                    <td><?= htmlspecialchars($r['ten_sp']) ?></td>
                                    <td><?= htmlspecialchars($r['ho_ten']) ?></td>
                                    <td><?= str_repeat('⭐', (int)$r['sao']) ?></td>
                                    <td><?= htmlspecialchars($r['binh_luan']) ?></td>
                                    <td><?= htmlspecialchars($r['ngay_danhgia']) ?></td>
                                    <td>
                                        <a href="delete_danhgia.php?id=<?= urlencode($r['id_danhgia']) ?>" class="btn btn-sm btn-outline-danger" onclick="return confirm('Bạn có chắc muốn xóa đánh giá này không?')">Xóa</a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
