...
if ($search !== '') {
    $stmt = $conn->prepare("
        SELECT km.*, sp.ten_sp 
        FROM khuyenmai km 
        JOIN sanpham sp ON km.id_sanpham = sp.id_sanpham 
        WHERE km.id_khuyenmai LIKE ? 
           OR sp.ten_sp LIKE ?
    ...
    <div class="admin-layout">
        ...
        <aside class="admin-sidebar">
            ...
            <nav class="admin-sidebar-nav">
                ...
                <a href="sanpham.php"
                    class="admin-nav-item <?php echo in_array(basename($_SERVER['PHP_SELF']), ['sanpham.php', 'add_sanpham.php', 'edit_sanpham.php']) ? 'active' : ''; ?>">
        ...
        <div class="admin-main">
            ...
            <header class="admin-topbar">
                <div class="admin-topbar-left">
                    <div class="admin-page-title">Khuyến mãi</div>
                </div>
            ...
            <main class="admin-content">
                <div class="card-panel">
                    <div class="card-panel-header d-flex flex-wrap justify-content-between align-items-center gap-2">
                        <div class="card-panel-title">Quản lý khuyến mãi</div>
                        ...
                        <div class="d-flex flex-wrap gap-2 align-items-center">
                            <form method="get" class="order-search">
                                <div class="input-group input-group-sm">
                                    <input type="text"
                                        name="search"
                                        class="form-control"
                                        placeholder="Tìm theo mã KM hoặc tên sản phẩm..."
                            ...
                            <a href="add_khuyenmai.php" class="btn btn-primary btn-sm">
                    ...
                    <div class="table-responsive mt-3">
                        <table class="table table-modern table-hover align-middle">
                            <thead>
                                <tr>
                                    <th style="width: 90px;">Mã KM</th>
                                    <th style="width: 100px;">Mã SP</th>
                                    <th>Tên sản phẩm</th>
                                    <th style="width: 180px;">Tên khuyến mãi</th>
                                    <th style="width: 90px;">Giảm %</th>
                                    <th style="width: 120px;">Giảm tiền</th>
                            ...
                            <tbody>
                                ...
                                <?php else: ?>
                                    <?php foreach ($khuyenmaiList as $km): ?>
                                        <tr>
                                            <td><?php echo htmlspecialchars($km['id_khuyenmai']); ?></td>
                                            <td><?php echo htmlspecialchars($km['id_sanpham']); ?></td>
                                            <td><?php echo htmlspecialchars($km['ten_sp']); ?></td>
                                            <td><?php echo htmlspecialchars($km['ten_km']); ?></td>
                                            <td>
                                                <?php
                                                echo $km['giam_gia_percent']
                                                    ? htmlspecialchars($km['giam_gia_percent']) . '%'
                                                    : '—';
                                                ?>
                                            </td>
                                            <td>
                                                <?php
                                                echo $km['giam_gia_tien']
                                                    ? number_format($km['giam_gia_tien'], 0, ',', '.') . '₫'
                                                    : '—';
                                                ?>
                                            </td>
                                            <td>
                                                <?php
                                                echo !empty($km['ngay_bat_dau'])
                                            ...
                                            <td>
                                                <?php
                                                echo !empty($km['ngay_ket_thuc'])
                                                    ? date('d/m/Y', strtotime($km['ngay_ket_thuc']))
                                                    : '—';
                                                ?>
                                            </td>
                                            <td>
                                                <a href="edit_khuyenmai.php?id=<?php echo urlencode($km['id_khuyenmai']); ?>"
                                                    class="btn btn-sm btn-outline-primary me-1">
                                                    Sửa
                                                </a>
                                                <a href="khuyenmai.php?delete=<?php echo urlencode($km['id_khuyenmai']); ?>"
