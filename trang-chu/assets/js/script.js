document.addEventListener('DOMContentLoaded', function() {
    ...
    if (document.querySelector('.build-pc-page')) {
        ...
        function updateSummaryAndTotal() {
            ...
            Object.keys(currentSelectedComponents).forEach(type => {
                ...
                if (component) {
                    ...
                    summaryItem.style.cssText = 'display:flex; align-items:center; margin-bottom:8px; font-size:0.9em;';
                    const img = document.createElement('img');
                    img.src = component.image || 'https://via.placeholder.com/40';
                    img.alt = component.name;
                    img.style.cssText = 'width:40px; height:40px; object-fit:contain; margin-right:10px; border:1px solid #eee;';
                    ...
                    summaryItem.append(img, nameDiv, priceDiv);
                ...
                .then(response => response.json()).then(data => {
                    ...
                    if (data.success && data.products.length > 0) {
                        data.products.forEach(product => {
                            ...
                            itemDiv.innerHTML = `<img src="${product.hinh_anh ? product.hinh_anh : 'https://via.placeholder.com/60'}" alt="${product.ten_sp}"><div class="info"><div class="name">${product.ten_sp} ${product.ten_thuonghieu ? '('+product.ten_thuonghieu+')' : ''}</div><div class="price">${parseInt(product.gia).toLocaleString('vi-VN')}₫</div></div><button class="btn-select-this-product" data-id="${product.id_sanpham}" data-name="${product.ten_sp}" data-price="${product.gia}" data-image="${product.hinh_anh || ''}">Chọn</button>`;
        ...
        if (closeModalButton) { closeModalButton.addEventListener('click', function() { modal.style.display = 'none'; }); }
        ...
        if (modalProductList) {
            modalProductList.addEventListener('click', function(event) {
                if (event.target.classList.contains('btn-select-this-product')) {
                    ...
                    currentSelectedComponents[currentEditingComponentType] = { id: selectedButton.dataset.id, name: selectedButton.dataset.name, price: selectedButton.dataset.price, image: selectedButton.dataset.image };
                    const targetRow = document.querySelector(`tr[data-component-type="${currentEditingComponentType}"]`);
                    if (targetRow) {
                        targetRow.querySelector('.selected-component-name .name-display').textContent = selectedButton.dataset.name;
                        const imgEl = targetRow.querySelector('.selected-component-name .selected-component-image');
                        imgEl.src = selectedButton.dataset.image || 'https://via.placeholder.com/40';
                        imgEl.alt = selectedButton.dataset.name;
                        imgEl.style.display = selectedButton.dataset.image ? 'inline-block' : 'none';
                        targetRow.querySelector('.selected-component-price').textContent = parseInt(selectedButton.dataset.price).toLocaleString('vi-VN') + '₫';
        ...
        if (btnAddBuildToCart) {
            btnAddBuildToCart.addEventListener('click', function() {
                const itemsToAdd = [];
                Object.keys(currentSelectedComponents).forEach(type => {
                    if (currentSelectedComponents[type]) {
                        itemsToAdd.push({ product_id: currentSelectedComponents[type].id, quantity: 1 });
                    }
                });
                if (itemsToAdd.length > 0) {
                    fetch('add_multiple_to_cart.php', { method: 'POST', headers: { 'Content-Type': 'application/json', }, body: JSON.stringify({ items: itemsToAdd }) })
                        .then(response => response.json()).then(data => {
                            if (data.success) { window.location.href = 'cart.php'; } else { alert('Lỗi khi thêm cấu hình vào giỏ: ' + (data.message || 'Vui lòng thử lại.')); }
                        }).catch(error => { console.error('Lỗi khi thêm cấu hình vào giỏ:', error); alert('Có lỗi xảy ra, không thể thêm cấu hình vào giỏ.'); });
                }
            });
        }
        if (typeof updateSummaryAndTotal === "function") { updateSummaryAndTotal(); }
    }
